#This code imolements a freightcar:
...
class FreightCar:
    ...
    def load_container(self, container):
        if container.ID != self.currentStation.ID:
            print(f"Container {container.ID} isn't in the same station as FreightCar: {self.ID}.")
            return False

        if len(self.containers) >= self.maxNumberOfAllContainers:
            print(f"FreightCar {self.ID} is at its maximum capacity.")
            return False
        ...
        if container.type == "HeavyContainer" and len([c for c in self.containers if c.type == "HeavyContainer"]) >= self.maxNumberOfHeavyContainers:
            print(f"FreightCar {self.ID} can't load anymore HeavyContainers.")
            return False
        ...
        if container.type == "RefrigeratedContainer" and len([c for c in self.containers if c.type == "RefrigeratedContainer"]) >= self.maxNumberOfRefrigeratedContainers:
            print(f"FreightCar {self.ID} can't load anymore RefrigeratedContainers.")
            return False
        ...
        if container.type == "LiquidContainer" and len([c for c in self.containers if c.type == "LiquidContainer"]) >= self.maxNumberOfLiquidContainers:
            print(f"FreightCar {self.ID} can't load anymore LiquidContainers.")
            return False
    ...
    def unload_container(self, container_id: int, current_station: Station):
        ...
        if container is None:
            ...
            print(f"Container {container_id} isn't in a FreightCar {self.ID}.")
    ...
    def calculate_fuel_consumption(self, destination_station):
        ...
        fuel_needed = self.currentStation.getDistance(destination_station) * self.double_fuelConsumptionPerKM + total_consumption

        return (fuel_needed <= self.double_fuel)

    def add_fuel(self, fuel_amount):
        if fuel_amount <= 0:
            print(f"Fuel amount must be positive! FreightCar {self.ID} fuel was not changed.")
            return
        self.double_fuel = self.double_fuel + fuel_amount
